...
# Define functions to create large models
def create_model_with_device(model_name, classification_word, hidden_layers, freeze_encoder=True):
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    return create_model(model_name, classification_word, hidden_layers, device, freeze_encoder)

def create_xlnet_large_model(classification_word, hidden_layers, freeze_encoder=True):
    return create_model_with_device("xlnet-large-cased", classification_word, hidden_layers, freeze_encoder)

def create_roberta_large_model(classification_word, hidden_layers, freeze_encoder=True):
    return create_model_with_device("roberta-large", classification_word, hidden_layers, freeze_encoder)

def create_bert_large_model(classification_word, hidden_layers, freeze_encoder=True):
    return create_model_with_device("bert-large-uncased", classification_word, hidden_layers, freeze_encoder)

def create_deberta_v3_large_model(classification_word, hidden_layers, freeze_encoder=True):
    return create_model_with_device("microsoft/deberta-v3-large", classification_word, hidden_layers, freeze_encoder)

def create_electra_large_model(classification_word, hidden_layers, freeze_encoder=True):
    return create_model_with_device("google/electra-large-discriminator", classification_word, hidden_layers, freeze_encoder)

def create_albert_xxlarge_model(classification_word, hidden_layers, freeze_encoder=True):
    return create_model_with_device("albert-xxlarge-v2", classification_word, hidden_layers, freeze_encoder)
...
def create_t5_large_model(classification_word, hidden_layers, freeze_encoder=True, model_name="t5-large"):
    try:
        t5_encoder = T5EncoderModel.from_pretrained(model_name)
        tokenizer = T5Tokenizer.from_pretrained(model_name)
...
model_variations = {
    ...
    "xlnet": {
        ...
        "xlnet_large": {
        "0_hidden": lambda cw, freeze_encoder=True: create_xlnet_large_model(cw, [], freeze_encoder),
        "1_hidden": lambda cw, freeze_encoder=True: create_xlnet_large_model(cw, [512], freeze_encoder),
        "2_hidden": lambda cw, freeze_encoder=True: create_xlnet_large_model(cw, [512, 256], freeze_encoder)
    },
    "roberta_large": {
        "0_hidden": lambda cw, freeze_encoder=True: create_roberta_large_model(cw, [], freeze_encoder),
        "1_hidden": lambda cw, freeze_encoder=True: create_roberta_large_model(cw, [512], freeze_encoder),
        "2_hidden": lambda cw, freeze_encoder=True: create_roberta_large_model(cw, [512, 256], freeze_encoder)
    },
    "bert_large": {
        "0_hidden": lambda cw, freeze_encoder=True: create_bert_large_model(cw, [], freeze_encoder),
        "1_hidden": lambda cw, freeze_encoder=True: create_bert_large_model(cw, [512], freeze_encoder),
        "2_hidden": lambda cw, freeze_encoder=True: create_bert_large_model(cw, [512, 256], freeze_encoder)
    },
    "deberta_v3_large": {
        "0_hidden": lambda cw, freeze_encoder=True: create_deberta_v3_large_model(cw, [], freeze_encoder),
        "1_hidden": lambda cw, freeze_encoder=True: create_deberta_v3_large_model(cw, [512], freeze_encoder),
        "2_hidden": lambda cw, freeze_encoder=True: create_deberta_v3_large_model(cw, [512, 256], freeze_encoder)
    },
    "electra_large_discriminator": {
        "0_hidden": lambda cw, freeze_encoder=True: create_electra_large_model(cw, [], freeze_encoder),
        "1_hidden": lambda cw, freeze_encoder=True: create_electra_large_model(cw, [512], freeze_encoder),
        "2_hidden": lambda cw, freeze_encoder=True: create_electra_large_model(cw, [512, 256], freeze_encoder)
    },
    "albert_xxlarge": {
        "0_hidden": lambda cw, freeze_encoder=True: create_albert_xxlarge_model(cw, [], freeze_encoder),
        "1_hidden": lambda cw, freeze_encoder=True: create_albert_xxlarge_model(cw, [512], freeze_encoder),
        "2_hidden": lambda cw, freeze_encoder=True: create_albert_xxlarge_model(cw, [512, 256], freeze_encoder)
    },
    "t5_large": {
        "0_hidden": lambda cw, freeze_encoder=True: create_t5_large_model(cw, [], freeze_encoder, "t5-large"),
        "1_hidden": lambda cw, freeze_encoder=True: create_t5_large_model(cw, [512], freeze_encoder, "t5-large"),
        "2_hidden": lambda cw, freeze_encoder=True: create_t5_large_model(cw, [512, 256], freeze_encoder, "t5-large")
    }
}
# Usage example:
